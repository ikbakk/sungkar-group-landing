...
async function main() {
  ...
  try {
    ...
    const importMatches = barrelContent.matchAll(
      /from\s+["']\.\/([^"']+)["']/g,
    );
    ...
    console.log(
      `Found ${referencedFiles.length} image references in barrel file`,
    );
    ...
    for (const ref of referencedFiles) {
      if (!imageFiles.includes(ref)) {
        ...
        const exists = imageFiles.some((f) =>
          f.startsWith(ref.replace(/\.[^.]+$/, "")),
        );
        if (!exists) {
