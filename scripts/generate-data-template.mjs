...
function getExistingCollectionSlugs() {
  ...
  const slugs = new Set();
  for (const d of readdirSync(dir, { withFileTypes: true }).filter((d) =>
    d.isDirectory(),
  )) {
    try {
      const main = JSON.parse(
        readFileSync(resolve(dir, d.name, "main.json"), "utf-8"),
      );
...
function getRegionCategory(region) {
  const map = {
    lombok: "Paket Wisata Lombok",
    "labuan-bajo": "Paket Wisata Labuan Bajo",
    sumbawa: "Paket Wisata Sumbawa",
  };
...
function getRegionCategoryEn(region) {
  const map = {
    ...
    "labuan-bajo": "Labuan Bajo Tour Packages",
    sumbawa: "Sumbawa Tour Packages",
  };
...
function getRegionCategoryAr(region) {
  const map = {
    lombok: "باكيجات سياحة لومبوك",
    "labuan-bajo": "باكيجات سياحة لابوان باجو",
    sumbawa: "باكيجات سياحة سومبawa",
  };
...
function getRegionCategoryMs(region) {
  const map = {
    lombok: "Pakej Pelancongan Lombok",
    "labuan-bajo": "Pakej Pelancongan Labuan Bajo",
    sumbawa: "Pakej Pelancongan Sumbawa",
  };
...
function getCollectionTitle(locale, slug) {
  const map = {
    ...
    "2-hari-1-malam": {
      id: "Paket 2 Hari 1 Malam",
      en: "2 Days 1 Night Package",
      ar: "باقة يومين وليلة",
      ms: "Pakej 2 Hari 1 Malam",
      zh: "两天一晚套餐",
    },
    "3-hari-2-malam": {
      id: "Paket 3 Hari 2 Malam",
      en: "3 Days 2 Nights Package",
      ar: "باقة 3 أيام وليلتين",
      ms: "Pakej 3 Hari 2 Malam",
      zh: "三天两晚套餐",
    },
    "4-hari-3-malam": {
      id: "Paket 4 Hari 3 Malam",
      en: "4 Days 3 Nights Package",
      ar: "باقة 4 أيام و 3 ليال",
      ms: "Pakej 4 Hari 3 Malam",
      zh: "四天三晚套餐",
    },
    "open-trip": {
      id: "Open Trip",
    ...
    "sailing-open-trip": {
      id: "Sailing Open Trip",
      en: "Sailing Open Trip",
      ar: "رحلة إبحار مفتوحة",
      ms: "Sailing Open Trip",
      zh: "帆船拼团游",
    },
    "sailing-private-trip": {
      id: "Sailing Private Trip",
      en: "Sailing Private Trip",
      ar: "رحلة إبحار خاصة",
      ms: "Sailing Private Trip",
      zh: "帆船私人游",
    },
  };
...
function getDuration(locale, slug) {
  ...
  return (
    map[slug]?.[locale] || map["3-hari-2-malam"]?.[locale] || "3 Hari 2 Malam"
  );
}
...
function buildLocaleFields(type, locale, { title, region, collectionSlug }) {
  ...
  switch (type) {
    case "package": {
      const colTitle = getCollectionTitle(
        locale,
      ...
      return {
        ...
        duration: dur,
        summary: isId
          ? "Jelaskan paket wisata ini..."
          : isEn
            ? "Describe this tour package..."
            : isAr
              ? "وصف الرحلة السياحية..."
              : isMs
                ? "Terangkan pakej pelancongan ini..."
                : "描述此旅游套餐...",
        highlights: isId
          ? ["Destinasi 1", "Destinasi 2", "Aktivitas 1"]
          : isEn
            ? ["Destination 1", "Destination 2", "Activity 1"]
            : isAr
              ? ["الوجهة 1", "الوجهة 2", "النشاط 1"]
              : isMs
                ? ["Destinasi 1", "Destinasi 2", "Aktiviti 1"]
                : ["目的地1", "目的地2", "活动1"],
        itinerary: isId
          ? ["Hari 1 - Kegiatan hari pertama"]
          : isEn
            ? ["Day 1 - First day activity"]
            : isAr
              ? ["اليوم 1 - نشاط اليوم الأول"]
              : isMs
                ? ["Hari 1 - Aktiviti hari pertama"]
                : ["第1天 - 第一天活动"],
        includes: isId
        ...
        perks: isId
          ...
          : isEn
            ...
            : isAr
              ...
              : isMs
                ? ["Kemudahan 1", "Kemudahan 2", "Kemudahan 3"]
                : ["设施1", "设施2", "设施3"],
        regionalHighlights: isId
          ? ["Keunggulan lokasi 1"]
          : isEn
            ? ["Location highlight 1"]
            : isAr
              ? ["ميزة الموقع 1"]
              : isMs
                ? ["Keistimewaan lokasi 1"]
                : ["地点亮点1"],
        description: isId
          ? "Deskripsi akomodasi..."
          : isEn
    ...
    case "vehicle":
      return {
        ...
        bestFor: isId
          ...
          : isEn
            ? ["Best for 1", "Best for 2"]
            : isAr
              ? ["مناسب لـ 1", "مناسب لـ 2"]
              : isMs
                ? ["Sesuai untuk 1", "Sesuai untuk 2"]
                : ["适合1", "适合2"],
        description: isId
          ? "Deskripsi kendaraan..."
          : isEn
...
async function generatePackage(slug, { title, region }) {
  ...
  const collectionSlug =
    colPick === "__custom__"
      ? await input({
          message: "Enter collection slug:",
          default: "3-hari-2-malam",
        })
      : colPick;

  const hasBoat = await confirm({
    message: "Include boat / cabin info?",
    default: false,
  });
  ...
  if (hasBoat) {
    main.boatName = await input({
      message: "Boat name:",
      default: "Nama Kapal",
    });
    main.boatType = await input({ message: "Boat type:", default: "Phinisi" });
    main.boatCapacity = await number({
      message: "Boat capacity (persons):",
      default: 12,
    });
    main.boatSpecs = [
    ...
    main.cabins = [
      {
        name: "Cabin Standard",
        description: "Kabin standar dengan AC",
        price: "Rp 3.500.000/orang",
        images: ["gallery/g-snorkel-3.webp"],
      },
    ];
  }
  ...
  writeFileSync(
    resolve(dir, "main.json"),
    JSON.stringify(main, null, 2) + "\n",
  );
  writeFileSync(
    resolve(dir, "locales.json"),
...
async function generateVehicle(slug, { title, region }) {
  ...
  const pricePerDay = await input({
    message: "Price per day (e.g. Rp 650.000):",
    default: "Rp 650.000",
  });
...
async function generateDestination(slug, { title, region }) {
  ...
  const main = {
    ...
    gallery: ["gallery/kuta-beach.webp"],
  };
  const locales = {};
  const localeOpts = { title, region };
  for (const loc of LOCALES) {
...
async function generateBlog(slug, title) {
  const description = await input({
    message: "Description:",
    default: "Deskripsi blog post...",
  });
  const readingTime = await number({
    message: "Reading time (minutes):",
    default: 5,
  });
  ...
  const selectedLocales =
    locales === "all" ? LOCALES : locales === "id-en" ? ["id", "en"] : ["id"];
...
async function main() {
  ...
  const type = await select({
    ...
    choices: [
      {
        name: "Package (Tour Package)",
        value: "package",
        description: "scripts/data/packages/{slug}/",
      },
      {
        name: "Accommodation",
        value: "accommodation",
        description: "scripts/data/accommodations/{slug}/",
      },
      {
        name: "Vehicle (Car Rental)",
        value: "vehicle",
        description: "scripts/data/car-rental/{slug}/",
      },
      {
        name: "Destination",
        value: "destination",
        description: "scripts/data/destinations/{slug}/",
      },
      {
        name: "Blog Post",
        value: "blog",
        description: "src/content/blog/{slug}/{locale}.mdx",
      },
      {
        name: "Travel Guide",
        value: "guide",
        description: "src/content/guides/{slug}/{locale}.mdx",
      },
    ],
  });
