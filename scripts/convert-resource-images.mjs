...
function pick(arr,n){return arr.filter(x=>fss.existsSync(path.join(ROOT,'src/assets/images',x.outRel))).slice(0,n).map(x=>x.outRel)}
function cabinType(s){s=slug(s); if(s.includes('ocean-double'))return 'ocean-double'; if(s.includes('ocean-twin'))return 'ocean-twin'; if(s.includes('royal'))return 'royal'; if(s.includes('signature')||s.includes('singature'))return 'signature'; if(s.includes('vvip'))return 'vvip'; if(s.includes('vip'))return 'vip'; if(s.includes('master'))return 'master'; if(s.includes('deluxe'))return 'deluxe'; if(s.includes('superior'))return 'superior'; if(s.includes('family'))return 'family'; if(s.includes('full-ocean'))return 'full'; if(s.includes('share'))return 'share'; if(s.includes('private'))return 'private'; if(s.includes('add')||s.includes('additional'))return 'add'; return 'cabin';}
function leadingNums(s){s=slug(s); const out=[]; const m=s.match(/^(?:cabin-)?(\d+)(?:-(\d+))?/); if(m){const a=+m[1], b=m[2]?+m[2]:a; for(let i=a;i<=b;i++) out.push(i);} return out;}
function cabinOrdinal(s){const m=slug(s).match(/(?:^|-)(\d+)(?:-|$)/g); if(!m) return 1; return Number(m.at(-1).replace(/-/g,''))||1;}
function tokenSet(s){return new Set(slug(s).split('-').filter(Boolean));}
function numericTokens(s){return slug(s).split('-').filter(t=>/^\d+$/.test(t)).flatMap(t=>[String(Number(t)), t.padStart(2,'0')]);}
const GENERIC_CABIN_TOKENS=new Set(['cabin','room','deck','lower','main','upper','view','bed','queen','single','double','triple','bunk','extra','private','bathroom','toilet','sharing','and','with','pax','orang','left','right','side','balcony']);
function nameTokens(s){return slug(s).split('-').filter(t=>t&&!/^\d+$/.test(t)&&!GENERIC_CABIN_TOKENS.has(t));}
function pickByName(cabinName,cabinBuckets){
  const wanted=nameTokens(cabinName);
  if(!wanted.length) return undefined;
  let matches=cabinBuckets.filter(bucket=>wanted.every(t=>tokenSet(bucket).has(t)));
  if(!matches.length) return undefined;
  const nums=numericTokens(cabinName);
  if(nums.length){
    const numbered=matches.filter(bucket=>{
      const tokens=tokenSet(bucket);
      return nums.some(n=>tokens.has(n)) || leadingNums(bucket).some(n=>nums.includes(String(n))||nums.includes(String(n).padStart(2,'0')));
    });
...
function chooseBucket(cabinName, cabinIndex, cabinBuckets, pkgSlug){
  ...
  const exact=slug(cabinName); let hit=cabinBuckets.find(b=>b===exact); if(hit) return hit;
  hit=pickByName(cabinName,cabinBuckets); if(hit) return hit;
  if(pkgSlug.includes('almadira')){ if(cabinType(cabinName)==='share') return cabinBuckets.find(b=>b.includes('cabin-3-share'))||cabinBuckets.find(b=>b.includes('share')); if(cabinType(cabinName)==='private') return cabinBuckets.find(b=>b.includes('cabin-1-2-share-private')); }
  if(pkgSlug.includes('ocean-pro')) return cabinBuckets.find(b=>b==='cabin')||cabinBuckets[0];
  const t=cabinType(cabinName); let same=cabinBuckets.filter(b=>cabinType(b)===t).sort((a,b)=>(leadingNums(a)[0]||999)-(leadingNums(b)[0]||999));
  if(t==='vvip' && same.length===0) same=cabinBuckets.filter(b=>cabinType(b)==='vip').sort((a,b)=>(leadingNums(a)[0]||999)-(leadingNums(b)[0]||999));
  if(t==='deluxe' && pkgSlug.includes('yukai')) return cabinName.toLowerCase().includes('upper') ? cabinBuckets.find(b=>b.includes('deluxe-upper')) : cabinBuckets.find(b=>b.includes('deluxe-main'));
  if(same.length===1) return same[0];
  const ord=cabinOrdinal(cabinName); if(same[ord-1]) return same[ord-1]; if(same.length) return same.at(-1);
  const nums=cabinBuckets.filter(b=>leadingNums(b).includes(cabinIndex+1)); if(nums.length===1) return nums[0];
  return undefined;
}
async function main(){const files=await walk(RESOURCE); const ignored={}, images=[]; for(const f of files){const ext=path.extname(f).toLowerCase(); if(IMG_EXT.has(ext)) images.push(f); else ignored[ext||'(none)']=(ignored[ext||'(none)']||0)+1;} const selected=LIMIT?images.slice(0,LIMIT):images; const assets=selected.map(outputFor); const boats=[...new Set(assets.map(a=>a.boatRaw))]; const pkgs=await packages(); const m=matchBoats(pkgs,boats); if(m.errors.length && mode!=='scan'){throw new Error('Boat mapping errors:\n'+m.errors.join('\n'));}
 ...
 const summary={mode,totalFiles:files.length,imageFiles:images.length,selectedImages:selected.length,ignored,boatErrors:m.errors,missingBoats:m.missing,assets:assets.map(a=>({boat:a.boat,source:a.srcRel,output:a.outRel,group:a.group,bucket:a.bucket,size:a.size??a.convert?.size})),packages:packageMappings};
