...
function ordinalMaps(names){const counts={}; return names.map(n=>{const t=cabinType(n); counts[t]=(counts[t]||0)+1; return {type:t,ordinal:counts[t]};});}
function chooseBucket(cabinName, cabinIndex, cabinBuckets, pkgSlug){
  ...
  const sameNum=same.filter(b=>leadingNums(b).includes(cabinIndex+1)); if(sameNum.length===1) return sameNum[0];
