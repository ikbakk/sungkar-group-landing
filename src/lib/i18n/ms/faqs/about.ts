export const aboutFaq = [
  {
    question: "Apakah Sungkar Group Indonesia?",
    answer:
      "Sungkar Group Indonesia adalah pengendali pelancongan tempatan yang berpangkalan di Lombok, fokus menyediakan pengalaman perjalanan yang selesa, terancang, dan tidak dapat dilupakan.",
  },
  {
    question: "Di manakah Sungkar Group beroperasi?",
    answer:
      "Kami berpangkalan di Lombok dan melayani pelbagai destinasi di Pulau Lombok, Kepulauan Gili, dan kawasan pelancongan lain di Nusa Tenggara Barat.",
  },
  {
    question: "Siapa yang biasanya menggunakan perkhidmatan Sungkar Group?",
    answer:
      "Perkhidmatan kami digunakan oleh pengembara individu, pasangan, keluarga, komuniti, sekolah, dan syarikat yang ingin mengatur perjalanan atau aktiviti berkumpulan.",
  },
  {
    question: "Apakah yang membezakan Sungkar Group dengan pengendali pelancongan lain?",
    answer:
      "We combine local knowledge, responsive service, and structured travel planning so guests can enjoy Lombok more comfortably and efficiently.",
  },
  {
    question: "Adakah Sungkar Group menyediakan perjalanan boleh suai?",
    answer:
      "Yes. We can help arrange itineraries tailored to group size, travel duration, tourism interests, and other specific needs.",
  },
  {
    question: "Bagaimana saya boleh menghubungi Sungkar Group?",
    answer:
      "You can reach our team via WhatsApp or the contact page to discuss your travel plans and tourism needs.",
  },
];
