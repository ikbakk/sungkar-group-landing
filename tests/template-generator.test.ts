...
describe("slugify", () => {
  it("converts title to slug", () => {
    const src = readFileSync(resolve(ROOT, "scripts/generate-data-template.mjs"), "utf-8");
...
describe("Generated JSON validity", () => {
  it("all package main.json files are valid JSON", () => {
    ...
    const dirs = readdirSync(dir, { withFileTypes: true })
      ...
      .map((d) => d.name);
    for (const slug of dirs) {
      expect(() => JSON.parse(readFileSync(resolve(dir, slug, "main.json"), "utf-8"))).not.toThrow();
      expect(() => JSON.parse(readFileSync(resolve(dir, slug, "locales.json"), "utf-8"))).not.toThrow();
    }
  });
});
